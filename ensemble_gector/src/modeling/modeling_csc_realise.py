...
from src.modeling.realise_sub_char_cnn import CharResNet, CharResNet1
from torch import nn
from transformers.models.bert import BertModel, BertPreTrainedModel
from utils.pinyin_util import _is_chinese_char, pho2_convertor
from src.loss.focal_loss import FocalCELoss


class ModelingCscRealise(BertPreTrainedModel):
    def __init__(self, config):
        super(ModelingCscRealise, self).__init__(config)
        self.config = config
        ...
        if config.add_detect_task:
            self.detect_cls = nn.Linear(
                config.hidden_size, config.type_vocab_size)
        if config.add_pinyin_task:
            self.pinyin_cls = nn.Linear(
                config.hidden_size, config.type_pinyin_vocab_size)
        self.init_weights()
    ...
    def _init_criterion(self):
        self.d_criterion = FocalCELoss(loss_labels_weights=[1, 3],
                                       gamma=2,
                                       ignore_index=-100)
        self.pinyin_criterion = FocalCELoss(loss_labels_weights=None,
                                            gamma=2,
                                            ignore_index=-100)

        self.c_criterion = FocalCELoss(loss_labels_weights=None,
                                       gamma=2,
                                       ignore_index=-100)
    ...
    def forward(self,
        ...
        bert_hiddens = self.bert(input_ids, attention_mask=attention_mask)[
            'last_hidden_state']
        ...
        d_logits = self.detect_cls(
            sequence_output) if self.config.add_detect_task else None

        pinyin_logits = self.pinyin_cls(
            sequence_output) if self.config.add_pinyin_task else None

        c_logits = self.classifier(sequence_output)
        loss = None
        if c_tags is not None:
            c_loss = self.c_criterion(
                c_logits.view(-1, self.config.vocab_size), c_tags.view(-1))
            d_loss = self.c_criterion(
                d_logits.view(-1, 2), d_tags.view(-1))
            pinyin_loss = self.c_criterion(
                pinyin_logits.view(-1, self.config.type_pinyin_vocab_size), pinyin_tags.view(-1))
            loss = c_loss + d_loss + pinyin_loss
            
        return d_logits, c_logits, pinyin_logits, loss
