...
class ModelingWoeS2eElectra(ElectraPreTrainedModel):
    ...
    def __init__(self,
                 config):
        super(ModelingWoeS2eElectra, self).__init__(config)
        self.config = config
    ...
    def forward(
        ...
        if self.config.append_crf:
            ...
            pred_labels = self.crf_layer.viterbi_decode(
...
class WordOrderErrorModelOnnx(ElectraPreTrainedModel):
    """合并了detector和Corrector
     "检错的话,预测对应位置的append, keep, replace, delete"
     "纠错的话,直接预测对应位置的CTC label的 1.6W标签"
    """
    ...
    def __init__(self,
                 config):
        super(WordOrderErrorModelOnnx, self).__init__(config)
        self.config = config
    ...
    def forward(
        ...
        if self.config.append_crf:
            ...
            if labels is None:
                # prediction mode
                return torch.LongTensor(pred_labels).cuda(), sequence_output
            crf_loglikelihood = self.crf_layer.forward(
                sequence_output, labels, attention_mask)
...
#     pretrained_dir = 'model/miduCTC_v3.3.0_0513/woe'
