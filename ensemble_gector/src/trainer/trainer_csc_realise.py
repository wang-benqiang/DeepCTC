...
from src.dataset.dataset_csc_realise import DatasetCscRealise
...
class TrainerCscRealise:
    ...
    def load_suite(self):
        "model"
        model = ModelingCscRealise.from_pretrained(
            self.in_model_dir)
    ...
    def train_step(self, batch_ds, ith_step):
        ...
        # 常规模式
        batch_c_logits, batch_src, batch_gold, batch_loss = self.model(
            **batch_ds
        )
    ...
    def evaluate(self, dataset_type='dev'):
        ...
        for batch_ds in dataloader:
            for k, v in batch_ds.items():
                batch_ds[k] = v.to(self.accelerator.device)
            batch_d_logits, batch_c_logits, batch_pinyin_logits, batch_loss = self.model(
                **batch_ds
            )
            batch_c_logits, batch_src, batch_gold, batch_loss = self.accelerator.gather(
                (batch_c_logits, batch_ds['input_ids'], batch_ds['c_tags'], batch_loss))
