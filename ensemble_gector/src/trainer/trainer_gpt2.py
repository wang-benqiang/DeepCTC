...
from src.dataset.dataset_gpt import DatasetGpt
...
class TrainerGpt2:
    ...
    def load_suite(self):
        "model"
        model = ModelingGpt2.from_pretrained(
            self.in_model_dir)
                ...
                if (step % ceil(self.check_val_every_n_epoch * len(self.train_dataloader)) == 0 or epoch_end_flag):
                    ...
                    if self.accelerator.is_local_main_process:
                        log_text = '[Evaluating] Epoch {}/{}, Step {}/{}, ' \
                                'epoch_loss:{}, '
                        logger.info(
                            log_text.format(epoch, self.epochs, step, steps, eval_epoch_loss,
                                            ))
                    if self.test_dataloader is not None:

                        test_epoch_loss = self.evaluate(
                            dataset_type='test')
                        if self.accelerator.is_local_main_process:
                    ...
                    if eval_epoch_loss >= 0:
                        #
                        if eval_epoch_loss < max_eval_loss:
                            max_eval_loss = eval_epoch_loss
                            # 重置early stop次数
                            ith_early_stop_time = 0
                            final_eval_scores_for_early_stop = []
                        else:
                            ...
                            if ith_early_stop_time >= self.early_stop_times:
                                ...
                                return
                        if self.test_dataloader is not None:
                            test_epoch_loss_str = str(
                                round(test_epoch_loss, 2))
                        else:
                            test_epoch_loss_str = 'None'
                        dev_epoch_loss_str = str(round(eval_epoch_loss, 2))
                        metric_str = 'epoch{},step{},testepochloss_{},devepochloss_{}'.format(epoch, step,
                                                                                              test_epoch_loss_str, dev_epoch_loss_str)
    ...
    def evaluate(self, dataset_type='dev'):
        ...
        for batch_ds in dataloader:
            ...
            batch_loss = self.accelerator.gather(batch_loss)
