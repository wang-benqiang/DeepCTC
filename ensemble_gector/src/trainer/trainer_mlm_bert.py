...
from src.dataset.dataset_bert_mlm import DatasetMlmBert
from src.metrics.metric_csc import f1_csc
from src.modeling.modeling_mlm_bert import ModelingBertMlm
...
class TrainerMlmBert:
    def __init__(self,
        ...
        self.tokenizer = BertTokenizerFast.from_pretrained(self.in_model_dir)
        ...
        if self._use_ema_train:
            self.ema_model = EMA(self.model, 0.9999, update_after_steps=100, update_every_steps=1)
        ...
        if self.accelerator.is_main_process:
            signature = inspect.signature(TrainerMlmBert)
            train_kwargs = {}
    ...
    def load_data(self) -> List[DataLoader]:
        ...
        train_dataloader = torch.utils.data.dataloader.DataLoader(
            train_ds, batch_size=self.batch_size, shuffle=True, num_workers=8, collate_fn=collate_fn)
        dev_dataloader = torch.utils.data.dataloader.DataLoader(
            dev_ds, batch_size=self.batch_size, shuffle=False, num_workers=8, collate_fn=collate_fn)
        if test_ds is not None:
            test_dataloader = torch.utils.data.dataloader.DataLoader(
                test_ds, batch_size=self.batch_size, shuffle=False, num_workers=8, collate_fn=collate_fn)

        return [train_dataloader, dev_dataloader, test_dataloader]

    def load_suite(self):
        "model"
        model = ModelingBertMlm.from_pretrained(
            self.in_model_dir)
        ...
        logger.info('num_training_steps:{}, warmup_steps:{}'.format(num_training_steps, self._warmup_steps))
    ...
    def train_step(self, batch_ds, ith_step):
        ...
        batch_logits, batch_loss = self.model(
            **batch_ds
        )
    ...
    def evaluate(self, dataset_type='dev'):
        ...
        for batch_ds in dataloader:
            ...
            batch_logits, batch_loss, batch_src, batch_gold = self.accelerator.gather(
                (batch_logits, batch_loss, batch_ds['input_ids'], batch_ds['labels']))
