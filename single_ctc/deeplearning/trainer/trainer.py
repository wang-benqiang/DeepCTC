...
from src.deeplearning.dataset.missing_dataset import DatasetMissing
...
class TrainerMissing:
    def __init__(self,
                 ...
                 train_db_dir='src/deeplearning/dataset/lm_db/db/ms_train',
                 train_db_gb_size=20,
                 dev_ds_pkl='dev_ds.pkl',
                 dev_db_dir='src/deeplearning/dataset/lm_db/db/ms_dev',
                 dev_db_gb_size=2,
                 test_ds_pkl='test_ds.pkl',
                 test_db_dir='src/deeplearning/dataset/lm_db/db/ms_test',
                 test_db_gb_size=1,
                 checkpoint_fp=None,
                 ):
        """[summary]
        ...
        Args:
            ...
            checkpoint_fp ([type], optional): [description]. Defaults to None.
        """
        ...
        self.train_db_dir = os.path.join(train_db_dir, '')[:-1]
        self.dev_db_dir = os.path.join(dev_db_dir, '')[:-1]
        self.test_db_dir = os.path.join(test_db_dir, '')[:-1]
        self.train_db_gb_size = train_db_gb_size
        self.dev_db_gb_size = dev_db_gb_size
        self.test_db_gb_size = test_db_gb_size
    ...
    def load_data(self) -> List[DataLoader]:
        ...
        if train_ds_pkl_fp is not None and os.path.exists(train_ds_pkl_fp):
            ...
            train_ds = DatasetMissing.load_ds_pkl(train_ds_pkl_fp)
        else:
            train_ds = DatasetMissing(
                self.train_file_dir,
                data_file_num_range_list=self.data_file_num_range_list,
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                pos_sample_gen_ratio=self.pos_sample_gen_ratio,
                max_dataset_len=self.max_dataset_length,
                ds_pkl_fp=train_ds_pkl_fp,
                db_dir=self.train_db_dir,
                db_gb_size=self.train_db_gb_size)
        ...
        if self.dev_file_dir is None:
            ...
            if dev_ds_pkl_fp is not None and os.path.exists(dev_ds_pkl_fp):
                # 先查看是否存在pkl,有则直接加载pkl
                dev_ds = DatasetMissing.load_ds_pkl(dev_ds_pkl_fp)
            else:

                dev_ds = DatasetMissing(
                    self.dev_file_dir,
                    tokenizer=self.tokenizer,
                    max_seq_len=self.max_seq_len,
                    ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                    ...
                    db_gb_size=self.dev_db_gb_size)
        ...
        if self.test_file_dir is not None:
            ...
            if test_ds_pkl_fp is not None and os.path.exists(
                    test_ds_pkl_fp):
                # 先查看是否存在pkl,有则直接加载pkl
                test_ds = DatasetMissing.load_ds_pkl(test_ds_pkl_fp)
            else:
                test_ds = DatasetMissing(
                    self.test_file_dir,
                    tokenizer=self.tokenizer,
                    max_seq_len=self.max_seq_len,
                    ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                    ...
                    db_gb_size=self.test_db_gb_size)
        else:
            test_ds = None
        ...
        logger.info('nodes_num:{}'.format(self.nodes_num))
    ...
    def get_config(self):
        ...
        config = {
           "model_dir": self.model_dir,
            ...
            "checkpoint_fp": self.checkpoint_fp,
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for step, batch_ds in track(enumerate(self.train_ds),
                ...
                if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                    ...
                    if eval_epoch_f1_nokeep > best_eval_score:
                        best_eval_score = eval_epoch_f1_nokeep

                        f1_str = str(round(test_epoch_f1_nokeep * 100,
                                           2)).replace('.', '_') + '%'
                        eval_epoch_f1_nokeep_str = str(round(eval_epoch_f1_nokeep * 100,
                                                             2)).replace('.', '_') + '%'
                        saved_fp = '{}/testf1_{}_devf1_{}.model'.format(
                            self.model_dir,f1_str, eval_epoch_f1_nokeep_str)
