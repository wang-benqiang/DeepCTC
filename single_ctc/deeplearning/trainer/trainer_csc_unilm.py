...
from src.deeplearning.dataset.csc_dataset_unilm import DatasetCscUnilm
...
from src.evaluate.metrics.metric_ctc import MetricForCtc
...
class TrainerCscUnilm:
    def __init__(self,
        ...
        if self.ddp_local_rank in (-1, 0):
            self.ts_board = SummaryWriter(log_dir='logs/ts_board')

    def load_label_dict(self):
        dtag_fp = os.path.join(self.ctc_label_vocab_dir,
                               'ctc_detect_2tags.txt')
    ...
    def load_model(self) -> ModelingUnilmCsc:
        # Realise
        model = ModelingUnilmCscBiAtt.from_pretrained(
            self.pretrained_model_dir)
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            epoch_end_flag = True if epoch > 1 else False # 每个epoch再验证
                    ...
                    if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                        ...
                        if eval_epoch_f1 >= 0:
                            ...
                            dev_f1_str = str(round(eval_epoch_f1 * 100,
                                                   2)).replace('.', '_') + '%'
                            metric_str = 'epoch{},ith_db{},step{},testf1_{},devf1_{},testloss{},devloss{},'.format(epoch, ith_train_db, step,
                                                                                                                   test_f1_str, dev_f1_str, round(test_epoch_c_loss, 4), round(eval_epoch_c_loss, 4))
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        if torch.cuda.is_available() and self.use_cuda:
            for k, v in batch_ds.items():
                batch_ds[k] = v.cuda() if 'raw' not in k else v
        ...
        # correct_h, l2r_h,  l2r_loss, correct_loss, total_loss
        c_logits, batch_loss = outputs[0], outputs[-1]
    ...
    def evaluate(self, data_type='dev'):
        ...
        self.model.eval()
        epoch_correct_loss = 0
        epoch_sim_loss = 0
        epoch_l2r_loss = 0
        epoch_r2l_loss = 0
        epoch_total_loss = 0
    
        epoch_src_texts, epoch_trg_texts, epoch_prd_texts = [], [], []
        ...
        for batch_ds in dataset:
            ...
            c_logits, l2r_h,  l2r_loss, correct_loss, batch_loss = outputs
            # 这里模型返回的logits已经是pred[1:]
            batch_loss = batch_loss.mean()

            batch_pred_tags = torch.argmax(c_logits, dim=-1).cpu().tolist()
            # correct
            # pred[1:] 去掉cls
            batch_pred_texts = [''.join(self.tokenizer.convert_ids_to_tokens(pred_tags[1:])[:len(
                trg)]) for pred_tags, trg in zip(batch_pred_tags, batch_ds['raw_trg'])]

            epoch_src_texts.extend(batch_ds['raw_src'])

            epoch_trg_texts.extend(batch_ds['raw_trg'])
            epoch_prd_texts.extend(batch_pred_texts)

            epoch_total_loss += batch_loss.item()
            epoch_correct_loss += correct_loss.item()
            epoch_l2r_loss += l2r_loss.item()
            # epoch_r2l_loss += r2l_loss.item()
            # epoch_final_loss += final_loss.item()
        
        
        self.ts_board.add_scalar('{}/epoch_total_loss'.format(data_type), epoch_total_loss, self._evaluate_time)
        # self.ts_board.add_scalar('{}/epoch_sim_loss'.format(data_type), epoch_sim_loss, self._evaluate_time)
        self.ts_board.add_scalar('{}/epoch_l2r_loss'.format(data_type), epoch_l2r_loss, self._evaluate_time)
        # self.ts_board.add_scalar('{}/epoch_r2l_loss'.format(data_type), epoch_r2l_loss, self._evaluate_time)
        self.ts_board.add_scalar('{}/epoch_correct_loss'.format(data_type), epoch_correct_loss, self._evaluate_time)
        # TODO:注视掉验证脚本seq2seq长度不一样, 先计算句子级别
        (pos_sent_precision, pos_sent_recall, pos_sent_f1),\
            (neg_sent_precision, neg_sent_recall, neg_sent_f1),\
            macro_sent_f1 = MetricForCtc.ctc_f1_sentence_level(
            src_texts=epoch_src_texts, trg_texts=epoch_trg_texts, pred_texts=epoch_prd_texts
        )

      
        return epoch_total_loss, neg_sent_precision, neg_sent_recall, neg_sent_f1
