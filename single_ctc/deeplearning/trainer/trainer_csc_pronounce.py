...
from src.deeplearning.dataset.csc_pronounce_bert_dataset import DatasetCscSeq2labelSimPinyin, GlobalVars
...
class TrainerCscPronounce:
    ...
    def load_data(self, ith_train_db) -> List[DataLoader]:
        ...
        train_ds = DatasetCscSeq2labelSimPinyin(
            tokenizer=self.tokenizer,
            max_seq_len=self.max_seq_len,
            max_dataset_len=self.max_dataset_len,
            db_dir=self.train_db_dirs[ith_train_db],
            enable_pretrain=self.enable_pretrain,
            char_confustion_iters=GlobalVars.char_confustion_iters,
            char_replace_time_distribution=GlobalVars.char_replace_time_distribution,
            char_replace_average_time=GlobalVars.char_replace_average_time
        )
        ...
        if self.dev_db_dirs is None or not os.path.exists(self.dev_db_dirs[0]):
            ...
            dev_ds = DatasetCscSeq2labelSimPinyin(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                max_dataset_len=self.max_dataset_len,
                enable_pretrain=self.enable_pretrain,
        ...
        if os.path.exists(self.test_db_dirs[0]):
            test_ds = DatasetCscSeq2labelSimPinyin(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                max_dataset_len=self.max_dataset_len,
    ...
    def load_model(self) -> ModelingCscPronounce:
        # Realise
        model = ModelingCscPronounce.from_pretrained(
            self.pretrained_model_dir) 
    ...
    def save_model(self, saved_dir):
        ...
        if self.ddp_local_rank in (-1, 0):
            ...
            char_replace_time_fp = open(os.path.join(self.model_dir, 'char_replace_time_fp.json'), 'w', encoding='utf8')
    
            json.dump(GlobalVars.char_replace_time_distribution.copy(), char_replace_time_fp, ensure_ascii=False, indent=2)
            logger.info('=========== char_replace_time_distribution saved  =============')
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for ith_train_db in range(len(self.train_db_dirs)):
                ...
                # if epoch > 1:
                    ...
                    # # refresh confusion distribution
                    # self.char_confustion_iters=self.train_ds.dataset.dataset.char_confustion_iters
                    # self.char_replace_time_distribution=self.train_ds.dataset.dataset.char_replace_time_distribution
                    # self.char_replace_average_time=self.train_ds.dataset.dataset.char_replace_average_time
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        if return_for_epoch_metric:
            batch_gold = batch_ds['correct_labels'].view(-1).tolist()
    ...
    def evaluate(self, data_type='dev'):
        ...
        for batch_ds in dataset:
            ...
            batch_gold = batch_ds['correct_labels'].view(-1).cpu().numpy()
            batch_pred = torch.argmax(c_logits,
            ...
            if self.model_config.add_detect_task:
                detect_batch_gold = batch_ds['detect_labels'].view(-1).cpu().numpy()
