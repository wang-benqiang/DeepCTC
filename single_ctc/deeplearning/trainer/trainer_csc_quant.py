...
from src.deeplearning.dataset.csc_quant_dataset import CscQuantDataset
...
class TrainerCscQuant:
    def __init__(self,
        ...
        if self.ddp_local_rank in (-1, 0) and not os.path.exists(self.model_dir):
        ...
        self.num_labels = self.model_config.vocab_size
        self.criterion, self.optimizer = self.load_suite()

    def load_data(self, ith_train_db_dir) -> List[DataLoader]:

      

      
        train_ds = CscQuantDataset(
            tokenizer=self.tokenizer,
            max_seq_len=self.max_seq_len,
            max_dataset_len=self.max_dataset_length,
            db_dir=self.train_db_dirs[ith_train_db_dir],
            )

        if self.dev_file_dirs is None:
            # 如果没有dev集,则会切分训练集
        ...
        if self.test_db_dirs is not None:
           
            test_ds = CscQuantDataset(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                max_dataset_len=None,
    ...
    def load_model(self) -> BertForCscQuantPunc:
            ...
            for ith_train_db in range(len(self.train_db_dirs)):
                ...
                for step, batch_ds in track(enumerate(self.train_ds),
                    ...
                    if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                        ...
                        eval_epoch_loss, eval_epoch_precision, eval_epoch_recall, eval_epoch_f1 = self.evaluate(
                            data_type='dev')
                        test_epoch_loss, test_epoch_precision, test_epoch_recall, test_epoch_f1 = self.evaluate(
                            data_type='test')
                        ...
                        if eval_epoch_f1 >= 0:
                            ...
                            f1_str = str(round(test_epoch_f1 * 100,
                                            2)).replace('.', '_') + 'p'
                            eval_epoch_f1_str = str(round(eval_epoch_f1 * 100,
                                                        2)).replace('.', '_') + 'p'
                            saved_dir = '{}/epoch:{},ith_db:{},step{},testf1_{}_devf1_{}'.format(self.model_dir, epoch, ith_train_db, step,
                                 f1_str, eval_epoch_f1_str)
    ...
    def evaluate(self, data_type='dev'):
        ...
        return epoch_loss, epoch_precision, epoch_recall, epoch_f1
