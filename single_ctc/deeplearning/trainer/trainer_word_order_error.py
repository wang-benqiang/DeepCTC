...
from src.deeplearning.dataset.word_order_error_dataset import DatasetWordOrderError
        ...
        if self.ddp_local_rank == 0:
            ...
            for ith_node, (node_gold, node_pred) in enumerate(zip(all_nodes_golds, all_nodes_preds)):
                ...
                if len(node_gold) == len(node_pred):
                    ...
                    logger.error('[node {} right on evaluation stage]: pred length:{}, gold length:{}'.format(ith_node, len(node_pred), len(node_gold)))
                else:
                    logger.error('[node {} error on evaluation stage]: pred length:{}, gold length:{}'.format(ith_node, len(node_pred), len(node_gold)))
