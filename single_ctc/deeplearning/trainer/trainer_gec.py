...
from src.deeplearning.dataset.gec_dataset import DatasetGec
...
class TrainerGEC:
    def __init__(self,
        ...
        self.d_criterion, self.c_criterion, self.optimizer = self.load_suite()
    ...
    def load_suite(self):
        ...
        return d_criterion, c_criterion, optimizer

    def load_model(self) -> ModelingGEC:
        # Realise
        model = ModelingGEC.from_pretrained(
            self.pretrained_model_dir)
                ...
                for step, batch_ds in track(enumerate(self.train_ds),
                    ...
                    while not trained_flag:
                        try:
                            ...
                            batch_d_loss, batch_c_loss, batch_gold, batch_pred = self.train_step(
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        if return_for_epoch_metric:
            ...
            return d_loss_item, c_loss.item(), list(batch_gold), list(batch_pred)
        else:

            return d_loss_item, c_loss.item(),  None, None
    ...
    def evaluate(self, data_type='dev'):
        ...
        for batch_ds in dataset:
            ...
            epoch_src = [self.tokenizer.convert_tokens_to_ids(
                '$KEEP')]*len(epoch_src)
