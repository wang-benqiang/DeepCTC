...
from src.deeplearning.dataset.mlm_dataset import MlmDataset
from src.deeplearning.modeling.modeling_mlm import ModelMlm
...
class TrainerMlm:
    def __init__(self,
        ...
        assert model_type in ('electra', 'bert')
        ...
        self.enable_random_mask = enable_random_mask
    ...
    def load_data(self, ith_train_db=0) -> List[DataLoader]:
        
        # RealiseDataset
        assert ith_train_db in range(len(self.train_db_dirs))
        ...
        train_ds = MlmDataset(
            ...
            enable_random_mask=self.enable_random_mask
        )
    ...
    def load_model(self) -> ModelMlm:
      

        model = ModelMlm.from_pretrained(
            self.pretrained_model_dir)
        if self.freeze_embedding:
                ...
                for step, batch_ds in track(enumerate(self.train_ds),
                    ...
                    while not trained_flag:
                        try:
                            ...
                            logger.error('Training step error continue')
                            logger.exception(e)
                            continue
                    ...
                    if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                        ...
                        if eval_epoch_f1 >= 0:
                            ...
                            saved_dir = os.path.join(self.model_dir, metric_str)
                            self.save_model(saved_dir)
                            early_stop_time = 0
                            final_eval_scores_for_early_stop = []
                            if eval_epoch_f1 >= 1:
                                logger.info(
                                    'Eval epoch f1-score has reached to 1.0, continue to train ')
                                # return
                        else:
                            early_stop_time += 1
                            ...
                            if early_stop_time >= self.early_stop_times:
                                ...
                                return 1
            if self.with_train_epoch_metric:
                (epoch_d_precision, epoch_d_recall, d_f1), (epoch_precision,
                                                            epoch_recall, epoch_f1) = f1_csc(epoch_gold_labels, epoch_preds)
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        c_logits, batch_loss = self.model(
