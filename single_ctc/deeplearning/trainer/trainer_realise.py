...
from src.deeplearning.dataset.realise_dataset import RealiseDataset
...
from src.evaluate.metrics.metric_csc import f1_csc
...
class TrainerRealise:
    def __init__(self,
                 ...
                 d_loss_labels_weights=None,
                 c_loss_labels_weights=None,
                 pinyin_loss_labels_weights=None,
                 focal_loss_gamma=2,
        ...
        self.d_loss_labels_weights = d_loss_labels_weights
        self.pinyin_loss_labels_weights = pinyin_loss_labels_weights
        self.c_loss_labels_weights = c_loss_labels_weights
        self.focal_loss_gamma = focal_loss_gamma
        self.loss_ignore_index = loss_ignore_index
        self.only_error_loss = only_error_loss
        ...
        self.d_criterion, self.c_criterion, self.pinyin_criterion, self.optimizer = self.load_suite()
    
    
        # if self.ddp_local_rank in (-1, 0):
        #     self._tensor_board = SummaryWriter()
    ...
    def load_data(self, ith_train_db) -> List[DataLoader]:
        ...
        train_ds = RealiseDataset(
            ...
            only_error_loss=self.only_error_loss
        )
    ...
    def load_suite(self):
        ...
        if self.model_config.add_detect_task:
            d_criterion = FocalCELoss(loss_labels_weights=self.d_loss_labels_weights,
                                      gamma=self.focal_loss_gamma,
                                      ignore_index=self.loss_ignore_index)
        else:
            d_criterion = None
        if self.model_config.add_pinyin_task:
            pinyin_criterion = FocalCELoss(loss_labels_weights=self.pinyin_loss_labels_weights,
                                           gamma=self.focal_loss_gamma,
                                           ignore_index=self.loss_ignore_index)
        else:
            pinyin_criterion = None
        c_criterion = FocalCELoss(loss_labels_weights=self.c_loss_labels_weights,
                                  gamma=self.focal_loss_gamma,
                                  ignore_index=self.loss_ignore_index)
        ...
        return d_criterion, c_criterion, pinyin_criterion, optimizer

    def load_model(self) -> SpellBertPho2ResArch3:
        # Realise
        model = SpellBertPho2ResArch3.from_pretrained(
            self.pretrained_model_dir)
    ...
    def get_config(self):
        ...
        config = {
            ...
            'd_loss_labels_weights': self.d_loss_labels_weights,  # 每个label的loss权重,
            'c_loss_labels_weights': self.c_loss_labels_weights,  # 每个label的loss权重,
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            epoch_d_loss, epoch_c_loss, epoch_pinyin_loss = 0, 0, 0
            "多个训练集拼接，一个epoch需要遍历一遍训练集"
                ...
                for step, batch_ds in track(enumerate(self.train_ds),
                    ...
                    while not trained_flag:
                        try:
                            ...
                            batch_d_loss, batch_c_loss, batch_pinyin_loss, batch_gold, batch_pred = self.train_step(
                    ...
                    epoch_pinyin_loss += batch_pinyin_loss
                    if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                        ...
                        eval_epoch_d_loss, eval_epoch_c_loss, eval_epoch_precision, eval_epoch_recall, eval_epoch_f1 = self.evaluate(
                            data_type='dev')
                        test_epoch_d_loss, test_epoch_c_loss, test_epoch_precision, test_epoch_recall, test_epoch_f1 = self.evaluate(
                            data_type='test')
                        log_text = '[Evaluating] Epoch {}/{}, Step {}/{}, ' \
                            'epoch_d_loss:{}, epoch_c_loss:{}, epoch_precision:{}, epoch_recall:{}, epoch_f1:{},  '
                        logger.info(
                            log_text.format(epoch, self.epochs, step, steps,
                                            eval_epoch_d_loss, eval_epoch_c_loss, eval_epoch_precision,
                        ...
                        logger.info(
                            log_text.format(epoch, self.epochs, step, steps,
                                            test_epoch_d_loss, test_epoch_c_loss, test_epoch_precision,
                        ...
                        if eval_epoch_f1 > 0:
                            ...
                            if early_stop_time >= self.early_stop_times:
                                ...
                                return 1
            if self.with_train_epoch_metric:
                (epoch_d_precision, epoch_d_recall, d_f1), (epoch_precision, epoch_recall, epoch_f1) = f1_csc(epoch_gold_labels,
                                                                                                              epoch_preds)
            ...
            if self.ddp_local_rank in (-1, 0):
                ...
                if self.eval_ratio_of_epoch < 1:
                    epoch_end_flag = True
                log_text = '[Training epoch] Epoch {}/{},' \
                    ' epoch_d_loss:{},epoch_c_loss:{}, epoch_precision:{}, epoch_recall:{}, epoch_f1:{}'
                logger.info(
                    log_text.format(epoch, self.epochs, epoch_d_loss, epoch_c_loss,
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        if torch.cuda.is_available() and self.use_cuda:
            ...
            batch_ds['pho_idx'] = batch_ds['pho_idx'].cuda()
            batch_ds['pho_lens'] = batch_ds['pho_lens'].cuda()
            if self.model_config.add_detect_task:
                batch_ds['d_tags'] = batch_ds['d_tags'].cuda()
            if self.model_config.add_pinyin_task:
                batch_ds['pinyin_tags'] = batch_ds['pinyin_tags'].cuda()

        self.optimizer.zero_grad()
        d_logits, c_logits, pinyin_logits = self.model(
            input_ids=batch_ds['input_ids'],
            attention_mask=batch_ds['attention_mask'],
            pho_idx=batch_ds['pho_idx'],
            pho_lens=batch_ds['pho_lens'],
        )
        c_loss = self.c_criterion(
            c_logits.view(-1, self.model_config.vocab_size), batch_ds['c_tags'].view(-1))

        if self.model_config.add_detect_task:
            d_loss = self.d_criterion(
                d_logits.view(-1, 2), batch_ds['d_tags'].view(-1))
            d_loss_item = d_loss.item()  # return value for calculating metric
        else:
            d_loss, d_loss_item = 0, 0

        if self.model_config.add_pinyin_task:
            pinyin_loss = self.pinyin_criterion(
                pinyin_logits.view(-1, self.model_config.type_pinyin_vocab_size), batch_ds['pinyin_tags'].view(-1))
            pinyin_loss_item = pinyin_loss.item()
        else:
            pinyin_loss, pinyin_loss_item = 0, 0

        batch_loss = d_loss + c_loss + pinyin_loss
        ...
        if return_for_epoch_metric:
            ...
            return d_loss_item, c_loss.item(), pinyin_loss_item, list(batch_gold), list(batch_pred)
        else:

            return d_loss_item, c_loss.item(), pinyin_loss_item,  None, None

    @torch.no_grad()
    def evaluate(self, data_type='dev'):
        # 分布式训练时, 外层调用前会确认节点为-1,0时, 才会做验证
        self.model.eval()
        epoch_d_loss, epoch_c_loss, epoch_pinyin_loss, epoch_precision, epoch_recall, epoch_f1 = 0, 0, 0, 0, 0, 0
        epoch_preds, epoch_gold_labels, epoch_src = [], [], []

        detect_epoch_preds, detect_epoch_gold_labels = [], []
        pinyin_epoch_preds, pinyin_epoch_gold_labels = [], []
        ...
        for batch_ds in dataset:
            ...
            # pinyin
            if self.model_config.add_pinyin_task:
                pinyin_batch_gold = batch_ds['pinyin_tags'].view(
                    -1).cpu().numpy()
                pinyin_batch_pred = torch.argmax(pinyin_logits,
                                                 dim=-1).view(-1).cpu().numpy()
                pinyin_batch_gold = pinyin_batch_gold[seq_true_idx].squeeze()
                pinyin_batch_pred = pinyin_batch_pred[seq_true_idx].squeeze()

                pinyin_epoch_gold_labels += list(detect_batch_gold)
                pinyin_epoch_preds += list(detect_batch_pred)
            ...
            epoch_d_loss += d_loss_item
            epoch_pinyin_loss += pinyin_loss_item
        ...
        pinyin_f1 = f1_score(pinyin_epoch_gold_labels,
                             pinyin_epoch_preds,
                             average='macro',
                             zero_division=0)
        logger.info('[{}, pinyin task]: with keep f1 score:{}%'.format(
            data_type, pinyin_f1*100))
        return epoch_d_loss, epoch_c_loss, c_precision, c_recall, c_f1
