...
from src.deeplearning.dataset.detect_csc_dataset import DatasetDetectCsc
...
class TrainerDetectCsc:
    def __init__(self,
                 ...
                 train_db_dir='src/deeplearning/dataset/lm_db/db/rdt_train',
                 train_db_gb_size=20,
                 dev_ds_pkl='dev_ds.pkl',
                 dev_db_dir='src/deeplearning/dataset/lm_db/db/rdt_dev',
                 dev_db_gb_size=2,
                 test_ds_pkl='test_ds.pkl',
                 test_db_dir='src/deeplearning/dataset/lm_db/db/rdt_test',
                 test_db_gb_size=1):
        """[summary]
    ...
    def load_data(self) -> List[DataLoader]:
        ...
        if train_ds_pkl_fp is not None and os.path.exists(train_ds_pkl_fp):
            ...
            train_ds = DatasetDetectCsc.load_ds_pkl(train_ds_pkl_fp)
        else:
            train_ds = DatasetDetectCsc(
                self.train_file_dir,
        ...
        if self.dev_file_dir is None:
            ...
            if dev_ds_pkl_fp is not None and os.path.exists(dev_ds_pkl_fp):
                # 先查看是否存在pkl,有则直接加载pkl
                dev_ds = DatasetDetectCsc.load_ds_pkl(dev_ds_pkl_fp)
            else:

                dev_ds = DatasetDetectCsc(
                    self.dev_file_dir,
                    tokenizer=self.tokenizer,
                    max_seq_len=self.max_seq_len,
                    ctc_label_vocab_dir=self.ctc_label_vocab_dir,
        ...
        if self.test_file_dir is not None:
            ...
            if test_ds_pkl_fp is not None and os.path.exists(
                    test_ds_pkl_fp):
                # 先查看是否存在pkl,有则直接加载pkl
                test_ds = DatasetDetectCsc.load_ds_pkl(test_ds_pkl_fp)
            else:
                test_ds = DatasetDetectCsc(
                    self.test_file_dir,
                    tokenizer=self.tokenizer,
                    max_seq_len=self.max_seq_len,
                    ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                ...
                if (step % self._eval_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                    ...
                    if eval_epoch_f1 > best_eval_score:
                        ...
                        eval_epoch_f1_str = str(round(eval_epoch_f1_nokeep * 100,
                                                      2)).replace('.', '_') + 'p'
                        saved_dir = '{}/testf1_{}_devf1_{}'.format(
                            self.model_dir, f1_str, eval_epoch_f1_str)
