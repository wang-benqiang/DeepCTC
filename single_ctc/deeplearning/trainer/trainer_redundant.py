...
from src.deeplearning.dataset.redundant_dataset import DatasetRedundant
...
class TrainerRedundant:
    ...
    def load_data(self) -> List[DataLoader]:
        ...
        if not os.path.exists(self.dev_db_dirs[0]):
            ...
            dev_ds = DatasetRedundant(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                max_dataset_len=None,
                db_dirs=self.dev_db_dirs
            )

        if os.path.exists(self.test_db_dirs[0]):
            test_ds = DatasetRedundant(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                ctc_label_vocab_dir=self.ctc_label_vocab_dir,
                max_dataset_len=None,
                db_dirs=self.test_db_dirs)
        else:
            test_ds = None
