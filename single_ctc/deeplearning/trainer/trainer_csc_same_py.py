...
from src.deeplearning.dataset.csc_same_py_seq2edit_dataset import DatasetCscSeq2editSamePy
from src.deeplearning.modeling.modeling_csc_seq2edit_same_py import ModelingCscSeq2EditSamePy
...
class TrainerCscSamePy:
    ...
    def load_data(self, ith_train_db) -> List[DataLoader]:
        ...
        train_ds = DatasetCscSeq2editSamePy(
        ...
        if self.dev_db_dirs is None or not os.path.exists(self.dev_db_dirs[0]):
            ...
            dev_ds = DatasetCscSeq2editSamePy(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                max_dataset_len=self.max_dataset_len,
                balanced_loss=False,
        ...
        if os.path.exists(self.test_db_dirs[0]):
            test_ds = DatasetCscSeq2editSamePy(
                tokenizer=self.tokenizer,
                max_seq_len=self.max_seq_len,
                max_dataset_len=self.max_dataset_len,
    ...
    def load_model(self) -> ModelingCscSeq2EditSamePy:
        # Realise
        model = ModelingCscSeq2EditSamePy.from_pretrained(
            self.pretrained_model_dir)
    ...
    def train_step(self, batch_ds, return_for_epoch_metric=True):
        ...
        c_logits, d_logits, batch_loss = self.model(
            ...
            pinyin_labels=batch_ds['py_tags'],
            edit_labels=batch_ds['c_tags'],
        )
