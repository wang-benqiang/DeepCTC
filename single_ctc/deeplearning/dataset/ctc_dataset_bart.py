...
class DatasetCtcBart(Dataset):
    ...
    def __init__(self,
        ...
        """
        super(DatasetCtcBart, self).__init__()
        self.max_seq_len = max_seq_len
        self.max_seq_len_nlg = max_seq_len + 8
        ...
        # voab id
        self._start_vocab_id = self.tokenizer.vocab['[START]'] # 输入句子的start token, 考虑到少字任务
        self._sos_vocab_id = self.tokenizer.vocab['[SOS]'] # 生成句子的start
        self._eos_vocab_id = self.tokenizer.vocab['[EOS]'] # 生成句子的end
    ...
    def parse_line_data_and_label(self, src, trg):
        ...
        src, trg = replace_punc_for_bert(
            '始'+src)[:self.max_seq_len - 2], replace_punc_for_bert(trg)[:self.max_seq_len_nlg - 1] # trg是用decoder预测,所以不用始开头, 只需要用eos结尾, 所以文本长度-1
        ...
        c_tags_pad_len = self.max_seq_len_nlg-len(c_tags)-1
        c_tags = c_tags + [self._eos_vocab_id] + [self._loss_ignore_id] * c_tags_pad_len
        
        # 计算detecot label
        keep_idx_list, replace_idx_list, delete_idx_list, missing_idx_list = self.match_ctc_idx(src, '始'+trg) # 对应前面处理src加了始字
