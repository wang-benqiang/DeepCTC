...
class ModelingCtcBart(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self._loss_fct = FocalCELoss(ignore_index=-100)
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=encoder_outputs[0][:,0,:].unsqueeze(dim=1),
            encoder_attention_mask=attention_mask[:, 0].unsqueeze(dim=1),
        ...
        if labels is not None:
            ...
            masked_lm_loss = self._loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
