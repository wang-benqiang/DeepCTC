...
# 严格解码 baseline
def span_decode(start_logits, end_logits, raw_text, id2ent,ide_logits=None):
    predict_entities = []
    start_logits=np.array(torch.softmax(torch.tensor(start_logits),-1))
    end_logits=np.array(torch.softmax(torch.tensor(end_logits),-1))
    end_pred = np.argmax(end_logits, -1)
    if ide_logits is not None:
        ide_logits = np.array(torch.softmax(torch.tensor(ide_logits), -1))
        ide_pred = np.argmax(ide_logits, -1)
        for idx, e_type in enumerate(ide_pred):
            if e_type == 1:
                start_logits[idx][1] = start_logits[idx][1] + 0 * ide_logits[idx][1]
            elif e_type == 2:
                start_logits[idx][2] = start_logits[idx][2] + 0 * ide_logits[idx][2]
            elif e_type == 3:
                # for i in range(3,16500,2):
                start_logits[idx,3:16500:2] = start_logits[idx,3:16500:2] + 0 * ide_logits[idx][3]
            elif e_type == 4:
                # for i in range(4, 16501, 2):
                start_logits[idx,4:16501:2] = start_logits[idx,4:16501:2] + 0 * ide_logits[idx][4]
    ...
    for idx,e_type in enumerate(end_pred):
        if e_type==2:
            start_logits[idx][1]=start_logits[idx][1]-0.4*end_logits[idx][2]
        if e_type==1:
            start_logits[idx][1]=start_logits[idx][1]+0.4*end_logits[idx][1]
    start_pred = np.argmax(start_logits, -1)
    for  s_type,e_type in zip(start_pred,end_pred):
        predict_entities.append((id2ent[s_type],s_type))
        # if (s_type!=1 and e_type==1):
        #     pass
        # elif (s_type==1 and e_type==2):
        #     # print(id2ent[s_type])
        #     pass
        # predict_entities[id2ent[s_type]].append((raw_text[i],i))
...
def span_evaluation(model, dev_info, device, ent2id):
    ...
    for tmp_pred in get_base_out(model, dev_loader, device):
        active_loss = tmp_pred[1]["attention_masks"].view(-1) == 1
        start_logits = tmp_pred[0][0].view(-1, len(ent2id))
        end_logits = tmp_pred[0][1].view(-1, 4)
        active_start_logits = start_logits[active_loss]
        active_end_logits = end_logits[active_loss]

        active_start_lables=np.argmax(active_start_logits.cpu().numpy(), -1)
        active_end_lables=np.argmax(active_end_logits.cpu().numpy(), -1)

        start_labels = tmp_pred[1]['start_ids'].view(-1)[active_loss].cpu().numpy()
        end_labels = tmp_pred[1]["end_ids"].view(-1)[active_loss].cpu().numpy()

        start_pred.extend(list(active_start_lables))
        end_pred.extend(list(active_end_lables))
        start_true.extend(list(start_labels))
        end_true.extend(list(end_labels))



        # tmp_start_logits=tmp_pred[0][0].cpu().numpy()
        # tmp_end_logits=tmp_pred[0][1].cpu().numpy()
        # start_lables = tmp_pred[1]
        # end_lables = tmp_pred
        #
    ...
    assert len(start_pred) == len(end_pred) == len(start_true)==len(end_true)

    start_acc=accuracy_score(start_pred,start_true)
    end_acc=accuracy_score(end_pred,end_true)


    return start_acc,end_acc
