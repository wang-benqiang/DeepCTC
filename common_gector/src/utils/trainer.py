...
def train(opt, model, train_dataset):
    ...
    model, device = load_model_and_parallel(model, opt.gpu_ids,opt.ckpt_dir)
    ...
    for epoch in range(opt.train_epochs):
        ...
        # for step, batch_data in enumerate(train_loader):
        #     with torch.no_grad():
        #         batch_data={"token_ids":batch_data["token_ids"],"attention_masks":batch_data["attention_masks"],"token_type_ids":batch_data["token_type_ids"]}
        #         for key in batch_data.keys():
        #             batch_data[key] = batch_data[key].to(device)
        #         model.eval()
        #         loss = model(**batch_data)[0]
