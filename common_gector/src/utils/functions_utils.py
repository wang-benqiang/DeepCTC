...
def ensemble_vote(entities_list, threshold=0.9):
    ...
    for _entities in entities_list:
        for _entity_idx,_entity in enumerate(_entities):
            for _ent_idx,_ent in enumerate(_entity):
                entities_dict[(_entity_idx,_ent_idx, _ent)] += 1
    result=[['$KEEP']*len(i) for i in entities_list[0]]

    for key in entities_dict:
        sentence_idx,token_index,_ent=key[0],key[1],key[2]
        if entities[(sentence_idx,token_index)]:
            if entities_dict[key]>entities[(sentence_idx,token_index)]:
                result[sentence_idx][token_index]=_ent
                entities[(sentence_idx, token_index)]=entities_dict[key]
        else:
            result[sentence_idx][token_index] = _ent
            entities[(sentence_idx, token_index)] = entities_dict[key]
        # if entities_dict[key] >= threshold_nums:
