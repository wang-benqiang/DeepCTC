...
class GecModel(BaseModel):
    def __init__(self,
        ...
        """
        super(GecModel, self).__init__(bert_dir, dropout_prob=dropout_prob)

        out_dims = self.bert_config.hidden_size

        mid_linear_dims = kwargs.pop('mid_linear_dims', 128)
        self.num_tags = num_tags
        self.num_dtags = num_dtags
        ...
        self.end_fc = nn.Linear(out_dims, num_dtags)
    ...
    def forward(self,
        ...
        if start_ids is not None and end_ids is not None and self.training:
            ...
            end_logits = end_logits.view(-1, self.num_dtags)
...
def build_model(bert_dir, **kwargs):
    ...
    model = GecModel(bert_dir=bert_dir,
                          num_tags=kwargs.pop('num_tags'),
                          num_dtags=kwargs.pop('num_dtags'),
